...
strptr_dcmp(const char **a, const char **b)
{
  ...
  for (sa = *a, sb = *b; (ca = *sa) != '\0' && (cb = *sb) != '\0'; sa++, sb++) {
    // advance sa, sb until *sa, *sb are both whitespace/alphanumeric
    if (ca != '\0' && ca != ' ' && !isalnum(ca))
      ca = *sa++;
    if (cb != '\0' && ca != ' ' && !isalnum(cb))
      cb = *sb++;
    // compare. this is still correct even if ca, cb are '\0'
    if (ca < cb)
      return -1;
    if (cb < ca)
      return 1;
  }
  // if we didn't break in the loop, then the strings are equal
  return 0;
}
